...
from app.models import shops, vehicle, services
...
def index():
    ...
    return render_template('index.html', config=config, services=services, vehicles=vehicles[:3])
...
def vehicle_save():
    ...
    vehicle_id = vehicle.insert(make=make, model=model, year=year, price=price, title=title, condition=condition)
